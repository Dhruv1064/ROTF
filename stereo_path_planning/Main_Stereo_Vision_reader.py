...
window_size = 1  # 5
min_disp = 2 # 2
num_disp = 130-min_disp   # 130 - min_disp
stereo = cv2.StereoSGBM_create(minDisparity = min_disp,
    numDisparities = num_disp,
    blockSize = window_size,
    uniquenessRatio = 10,
    speckleWindowSize = 200,  #100
    speckleRange = 32,  #32
    disp12MaxDiff = 5,  #5
    preFilterCap = 5,  #5
    P1 = 8*3*window_size**2,  # *5
    P2 = 32*3*window_size**2)    # *5
...
while True:
    with Pool(processes=2) as pool :
        ...
        # filt_Color= cv2.applyColorMap(filteredImg,cv2.COLORMAP_PINK)

        cv2.imshow('Filtered Color Depth', filteredImg)
        thresh = 130
        im_bw = cv2.threshold(filteredImg, thresh, 255, cv2.THRESH_BINARY)[1]
